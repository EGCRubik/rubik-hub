...
class FakenodoService(BaseService):
    ...
    def publish_deposition(self, deposition_id: int) -> dict:
        ...
        try:
            ...
            self._add_version(deposition_id, label, dep.doi, {"metadata_changed": False, "file_changed": True, "comment": "Published deposition"})
        except Exception:
            pass
        return {"state": "done", "submitted": True, "doi": dep.doi}

    def _add_version(self, deposition_id: int, version_label: str, doi: str = None, changes: dict = None) -> dict:
        dep = self.get_deposition_obj(deposition_id)
        if not dep:
            return {}
        meta_data = dep.meta_data or {}
    ...
    def append_version(self, deposition_id: int, version_major: int, version_minor: int, doi: Optional[str] = None) -> dict:
        return self._add_version(deposition_id, f"{version_major}.{version_minor}", doi)
    ...
    def set_dataset_version(self, deposition_id: int, version_major: int, version_minor: int, append_to_versions: bool = True) -> dict:
        ...
        if append_to_versions:
            self._add_version(deposition_id, label, dep.doi)
        return meta_data
